...
func (rl *RateLimiter) Allow(ip string) (bool, int, time.Time) {
	...
	if !exists {
		rl.mu.Lock()
		// Double-check: another goroutine may have created the bucket
		bucket, exists = rl.buckets[ip]
		if !exists {
			now := time.Now()
			bucket = &TokenBucket{
	...
	if bucket.tokens > 0 {
		bucket.tokens--
		return true, bucket.tokens, bucket.refillAt
	}

	return false, 0, bucket.refillAt
}
...
func RateLimitMiddleware(limiter *RateLimiter, paths ...string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// Check rate limit
		allowed, remaining, refillAt := limiter.Allow(clientIP)
		...
		if !allowed {
			...
			retryAfter := int(time.Until(refillAt).Seconds())
